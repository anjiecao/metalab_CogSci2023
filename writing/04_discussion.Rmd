...
categorized_age_pred_df <- all_age_pred %>% 
  ...
  mutate(
    ci.lb = case_when(
      ds_name %in% c("Switch task", "Statistical word segmentation") ~ -ci.lb,
      TRUE ~ ci.lb
    ), 
    ci.ub = case_when(
      ds_name %in% c( "Switch task", "Statistical word segmentation") ~ -ci.ub,
      TRUE ~ ci.ub
    )
  ) %>% 
